...
**Sibling spec:** these predicates prove things *about* the credentials in the [DTG Credentials Core Specification](https://trustoverip.github.io/dtgwg-cred-spec/) (VRC/VMC/VIC/VPC/VEC/VWC, R/M/C/P-DID). That spec is format-agnostic on ZK and defers the ZK layer to us; [`CRED-SPEC-COHERENCE.md`](./CRED-SPEC-COHERENCE.md) maps each predicate to its credential anchor.
...
| `multi-issuer/` | X8 | §12.5 executable: issuer registry with ε + dependency classes, independence collapse (connected components, always weakens the bound), per-show distinctness nullifiers, k-tier vocabulary at build AND verify, statement reports effectiveK never raw k, corrupted-issuer degradation; **biometric dependency axis first-class (2026-08-25, upstream discussion #12): `model:`/`vendor:`/`enrolment-data:` + X8-B1..B4 — legally separate issuers sharing a model collapse to one component** | 🟢 14/14 |
