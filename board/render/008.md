# CARD 008 · Blinded binder (taskContext without a durable correlator)
...
## Method
1. C opens to (taskContext, u) for the taskContext the verifier supplies — route 1; or C opens to a PRF of (holder secret, context descriptor) that the verifier can recompute for this context and no other — route 2 → `commitment-open`
...
## Adversary
- route 1: a verifier not party to the exchange learns nothing from C; a verifier party to it learns only what it already held — against: verifier
- verifiers in different contexts cannot link presentations through the binder — route 1 emits nothing reusable, route 2 emits a value scoped to one context descriptor — against: verifiers-colluding
