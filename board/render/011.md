# RECORD 011 · Pairwise edge (VRC possession, directed personas shown, pairwise identifiers hidden)

**kind:** composed · **state:** `specified` · **priority:** P2 · **owner:** mitchuski · **composes:** 003 ∧ 004 ∧ 006 ∧ 007
...
2. each disclosed persona identifier is co-controlled with its hidden pairwise identifier (record 007) — the presenter’s from their own secret, the counterparty’s from the counterparty’s attestation → `key-binding` (record 007)
3. the presenter’s presentation key derives from the secret behind their pairwise identifier → `key-binding` (record 004)
4. the VRC handle is not revoked at epoch → `non-revocation` (record 006)
5. bound to one transcript → `transcript-bind` (record 003)
...
| 2026-08-28 | specified | board/specify | mitchuski | cred-spec construction 1 + #9 F post |
