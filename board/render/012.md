# RECORD 012 · Intentional correlation — one controller across k credentials

**kind:** composed · **state:** `specified` · **priority:** P2 · **owner:** mitchuski · **composes:** 003 ∧ 006 ∧ 007
...
## Relation
1. for each pair (identifier_1, identifier_i), i = 2..k: both open to the same s — k−1 common-control clauses sharing one witness → `key-binding` (record 007)
2. no credential handle is in the set under rl_root at epoch → `non-revocation` (record 006)
3. the whole show is bound to transcriptDigest → `transcript-bind` (record 003)
...
- the issuer of any one credential in the show learns nothing about the others from the proof; the revocation-state fetch must not be a per-holder query (record 006 C3) — against: issuer-verifier-colluding, registry-operator
...
| 2026-09-05 | specified | board/specify | mitchuski | composed from 007 + 006 + 003 under one transcript; disclosure set and negative space written fresh (composition rule) |
