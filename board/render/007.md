...
## Adversary
- no cross-presentation handle: the proof is transcript-bound and emits no identifier-derived value; two verifiers comparing proofs learn only what the enclosing records disclosed to each — against: verifier, verifiers-colluding
...
| 2026-09-05 | specified | board/specify | mitchuski | specified from PR #30's §Community-Anchored text + #9 + the lab's key-binding gadget shape; cost line labelled conjecture per drafting rule 4 |
