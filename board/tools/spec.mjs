...
export const CARD_DEFS = {
  ...
  'record state': 'The evidence state of a construction record — requested, carded, constructed, run, vetted, published — advanced monotonically as evidence accumulates; a record is informative until vetted and normative only when published.',
...
export function renderRecipe(card, all) {
  ...
  const stateNote = {
    ...
    published: 'This record is at state `published`: vetted and published by the task force. Normative.',
  }[card.state];
...
export function writeCookbook(cards, gadgets = null) {
  ...
  const written = [];
  for (const [t, d] of Object.entries(GADGET_DEFS)) { const f = `g-gadget-${slug(t)}.md`; writeFileSync(join(TERMS, f), renderTermFile(t, d, [`${t} gadget`])); written.push(f); }
  for (const [t, d] of Object.entries(ROLE_DEFS)) { const f = `g-role-${slug(t)}.md`; writeFileSync(join(TERMS, f), renderTermFile(t, d)); written.push(f); }
  for (const [t, d] of Object.entries(CARD_DEFS)) { const f = `g-${slug(t)}.md`; writeFileSync(join(TERMS, f), renderTermFile(t, d, t === 'horizon' ? ['Horizon'] : [])); written.push(f); }
