...
>
> **Runtime traces:**
> - `runtimes/07-trust-graph-formation/` — the runtime this chronicle records; `node test.mjs` → 11/11
> - `runtimes/CRED-SPEC-COHERENCE.md` + `runtimes/CRED-SPEC-OPPORTUNITIES.md` — the coherence pass
> - `runtimes/01-uniqueness-nullifier/` — the nullifier the personhood gate reuses
...
**Companion artefacts:**
- Runtime + forms-spec: `runtimes/07-trust-graph-formation/` (NOTES.md + src + 11/11 tests)
- Coherence map: `runtimes/CRED-SPEC-COHERENCE.md`
- Opportunity register: `runtimes/CRED-SPEC-OPPORTUNITIES.md`
- Dream-cycle record (KG voice): the KG-voice dream record (maintainer’s private suite)
...
The exploration side moved to its own root: this repository (the research root) now holds the **Predicate &
...
per-surface status lives in `INTEGRATION-MAP.md`.
