...
2. **The Predicate & Assurance-Boundary Decision Document** (`../predicate-assurance-boundary-decision.md`,
...
|---|---|---|---|---|---|
| [O1-deferred-zk-layer.md](./O1-deferred-zk-layer.md) | O1 | §25 gate · §7 profiles | 🔴 | design doc | DTG ZKP V1.0 = *the* conformant binding for the spec's two ZK constructions |
| [O2-phc-by-nullifier.md](./O2-phc-by-nullifier.md) | O2 | PR-UNQ (§13) | 🔴 | 🟢 **CIRCUIT** (`runtimes/circom-gadget/` 8/8, groth16: 11.5k constraints · 640 ms prove · 8 ms verify) | scoped reuse detection is the cryptographic leg of a PHC — narrow claim, exactly stated |
| [O4-registry-zk-revocation.md](./O4-registry-zk-revocation.md) | O4 | PR-ISS (§12) · §22 | 🟠 | design doc (+rt 07 rule) | how a ZK proof reads a trust registry — snapshots, revocation, edge-admissibility |
| [O3-idvc-assurance-profile.md](./O3-idvc-assurance-profile.md) | O3 | PR-LIV (§10) · §18 schema | 🟠 | design doc | the IDVC as the MLP's qualifying attestation — schema as the shared boundary |
| [VWC-witness-seat.md](./VWC-witness-seat.md) | rt 07 next | §15 transcript · VWC | 🟠 | design doc → runtime ext | the third seat in the dream cycle — a transcript-bound witness on the collision |
| [O7-agent-card-zk.md](./O7-agent-card-zk.md) | O7 | PR-DEL (§17) · §7.3 | 🟠 | design doc | proofs *over* Agent cards as PR-DEL delegation evidence — never key-control alone |
| [O9-vrc-promise-bundle.md](./O9-vrc-promise-bundle.md) | O9 | §18 schema governance | 🟠 | design doc | promise-theoretic / economic VRC semantics as an extension the minimal schema carries |
| [O5-O6-O8-supporting.md](./O5-O6-O8-supporting.md) | O5·O6·O8 | PR-FRE (§15) · §6.2 · §24/§29 | 🟡 | design notes | transcript conformance harness · pools cross-pollination · PQ path |
...
|---|---|---|---|---|
| [X1-conformance-fixtures.md](./X1-conformance-fixtures.md) | §26/§26.1 · §25 item 10 | 🔴 | 🟢 reference **v2** (`runtimes/fixtures/` 13/13 · register = 95 entries, one vocabulary lab-wide · §26.1 **11/11**) | the fixtures ARE the interop layer — vectors + a normative rejection-reason register + lint vectors for negative meanings |
| [X2-context-legibility-instrument.md](./X2-context-legibility-instrument.md) | §6.8 · §2.5 · §27.4 | 🟠 | 🟢 instrument (`runtimes/context-card/` 13/13) | the context card derived from the §6.2 descriptor itself — legibility as derivation, not audit; MyTerms' verifier-side twin |
| [X3-trust-task-composition.md](./X3-trust-task-composition.md) | §2.6 · §27.2 · §15.2 | 🔴 | 🟢 reference (`runtimes/show-composition/` 10/10) | the show is one transcript — joint disclosure is wider than the union; governed bundle profiles over à-la-carte requests |
| [X4-observable-event-minimisation.md](./X4-observable-event-minimisation.md) | §20 · §19 · §6.6 | 🟠 | 🟢 reference (`runtimes/quiet-presentation/` 8/8) | per-observer leakage budgets, the "quiet presentation" tier, and a log-field register — pools traffic-analysis discipline imported |
| [X5-recovery-rotation.md](./X5-recovery-rotation.md) | §29 · §13.4 · §22 | 🔴 | 🟢 reference (`runtimes/rotation/` 10/10) | routine rotation never touches enrolment; catastrophic recovery gets recovery-domain nullifiers — no old→new correlator |
| [X6-assurance-horizons-erosion-clocks.md](./X6-assurance-horizons-erosion-clocks.md) | §5.9 · §22.1 · §2.4 | 🟡 | 🟢 reference (`runtimes/erosion-record/` 8/8) | §22.1's ten clocks sorted into certification vs erosion families; "for how long" as a rate, not a cliff — the PVM reading |

**Third wave (2026-07-18)** — the last uncovered §-anchors plus X5's deferred pattern:

| Doc | Decision-doc anchor | Leverage | Ladder | One line |
|---|---|---|---|---|
| [X7-mediated-proving-profile.md](./X7-mediated-proving-profile.md) | §21 · §6.6 · §23 | 🟠 | 🟢 reference (`runtimes/mediator/` 9/9) | mediator taxonomy (blind/semi-trusted/prohibited honeypot), the four-exit declared-downgrade state machine, P-ISOLATE/P-FORGET as testable controls — completes X4's deferred budget row |
| [X8-multi-issuer-aggregation.md](./X8-multi-issuer-aggregation.md) | §12.5 · §29 · §2.4 | 🟠 | 🟢 reference (`runtimes/multi-issuer/` 14/14, biometric axis 2026-08-25) | ∏ε_i as a theorem with assumptions explicit; the independence register makes A2 checkable; k IS the disclosure (governed k-tiers); honest about PR-PER vs PR-LIV |
...
summary of the whole programme is `../BRIEFING-2026-07-18-zkp-explorations.md`.
