...
version: 0.1.0-draft
date: 2026-07-18
status: RESEARCH-ROOT SPEC DRAFT — promotion of the 2026-06-28 LAN workshop chronicle;
...
ceremony tests whether the party at the door has *read the room's terms and understood
them* — and only then opens graduated access. The lintel over the door (Gödel-deepened,
Mitch's correction that the danger of "perfect" is structural, not pragmatic):
...
witness artifact, a broker, an externally-anchored record — must be able to attest the
ceremony happened, without becoming the authority for what it means.
...
1. **unresolved → resolved** — *naming*. The joining agent OFFERS its address (consent,
   ...
   is worse than never — the david.local case is the proof.
2. **resolved → handshook** — *the gateway page*. The agent is pointed at a gateway
   document stating the room's terms. Reading it is the handshake; understanding begins
   here, not at a login form. The front door serves a catch-all so near-misses degrade
   soft (302 to the gateway), never hard-fail.
3. **handshook → exchanged** — *understanding demonstrated*. The agent answers from the
   gateway's content (comprehension, not copy) — the same primitive as the Gatehouse
   understanding challenge and the sigil-gate proverb, in its consent-first peer form.
4. **exchanged → sealed** — *the agreement*. The seal is a **bilateral MyTerms
   ...
   (agents/devices/credentials are the fabric around it, never parties). The individual
   PROPOSES terms from a standard roster (acceptance-before-proposal invitation
   pattern); three registers stay in correspondence — plain language, legal (Customer
   Commons canonical), machine-readable (JSON-LD/W3C DPV).
...
envelope mapping — and the registry already contains the pattern for step 5:
`vault/proxy-login` ("exercise a credential without releasing it") is precisely the
broker move, with `exposure.discloses` distinguishing a session outcome from released
...
- **agent-admission/*** — the SUPERVISED variant: a regulator/supervisor gate, two-gate
  rule (human approval ∧ understanding), derived scope, bilateral VRC, audit chain.
- **understanding/*** — the CONSENT-FIRST PEER variant: no supervisor; the host's terms
  and the joiner's proposal meet as equals under 7012; the seal is an agreement, not a
...
OUTSIDE the agreement = the Gödel witness. A mature ecosystem uses both — the agreement
inside, the credential outside, neither substituting for the other. This is the
credential/artifact wall again, seen from the agreement side.
...
1. **Seal format.** Pin the machine-readable register: which roster (SD-BASE/PDC-AI…),
   which DPV terms, how the agreement is referenced from `understanding/seal` (by
   digest, presumably — same-bytes-same-hash).
2. **Understanding scoring.** The peer variant currently trusts the host's judgment;
   the witness-draw mechanism from the admission ceremony (criteria drawn from the
   hash of the joiner's own submission) ports cleanly and removes the grooming vector.
3. **Broker generalisation.** State the grant-mediation pattern abstractly enough that
   `vault/proxy-login` reads as one instance of it (their spec) and the cookie jar as
   another (ours) — that's the interop story, not the cookie.
4. **The state machine as normative text** — five states, monotone, the two 502 lessons
